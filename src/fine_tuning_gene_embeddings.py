...
def train(model: nn.Module, loader: DataLoader) -> None:
    ...
    for batch, batch_data in enumerate(loader):
        ...
        with torch.cuda.amp.autocast(enabled=config.amp):
            ...
            # loss = loss + config.dab_weight * loss_dab #dabloss是关于batch integration的loss 不用加进来总的loss里面
...
if __name__ == "__main__":
    ...
    parser.add_argument("--load_model", required=True, help="The folder path of pretained scGPT model")
    ...
    print('############ ------------- SpaCCC --------------- ############')
    print('>>> arguments <<< \n', args)
    print('>>> loading hyperparameter and data <<< ', time.ctime())
    ...
    hyperparameter_defaults = dict(
        ...
        dab_weight=0, # DAR objective weight for batch correction  不需要这个参数
    ...
    print('>>> settings for input and preprocessing <<< ', time.ctime())
    ...
    save_dir = Path(f"{save_dir}/dev_{dataset_name}-{time.strftime('%b%d-%H-%M')}/")
    save_dir.mkdir(parents=True, exist_ok=True)
    print(f"save to {save_dir}")
    logger = scg.logger
    scg.utils.add_file_handler(logger, save_dir / "run.log")
    print('>>> Load data <<< ', time.ctime())
    # 2.1 Load the PBMC 10K data
    if dataset_name == dataset_name:
        filename = filename #3798 22240
    ...
    print('>>> Pre-process data <<< ', time.ctime())
    # 2.3 Pre-process the data   set up the preprocessor, use the args to config the workflow
    preprocessor = Preprocessor(
        ...
        log1p=data_is_raw,  # 4. whether to log1p the normalized data 数据集已经做过预处理lognomallized,在前面设置参数为false
    ...
    criterion_dab = nn.CrossEntropyLoss()#这个是关于batch的损失函数 用不到
    ...
    for epoch in range(1, config.epochs + 1):
        ...
        if epoch % config.save_eval_interval == 0 or epoch == config.epochs:
            ...
            #     str(save_dir / f"embeddings_celltype_umap[cls]_e{best_model_epoch}.pdf"),
            #     caption=f"celltype avg_bio epoch {best_model_epoch}",
            # )
    ...
    print('>>> Save the best model <<< ', time.ctime())
    # save the best model
